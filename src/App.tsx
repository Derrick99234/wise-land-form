...
function App() {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const response = await fetch(
      "https://wise-land-form-api.onrender.com/form/create",
      {
        method: "POST",
        headers: {
    ...
    <>
      <main className="">
        <div className="flex justify-center items-center min-h-screen gap-10 flex-wrap">
          <img src={busLogo} className="max-w-md w-full" />
          <form
            className="max-w-md w-full bg-black/5 p-5 px-8 mx-4"
            onSubmit={(e) => handleSubmit(e)}
          >
            <h2 className="text-2xl font-bold uppercase text-slate-900 mb-3">
              Ride to Work for Free - Your Morning, Made Easy!
            </h2>
            <FormInput
              label="Full Names"
      ...
      <footer>
        <p className="text-center text-slate-900 py-5">
          © {new Date().getFullYear()}, All rights reserved | Sponsored by
          Prophet Isaiah{" "}
          <a
            href="https://macwealthfreestore.com/list/71b6e174e5"
